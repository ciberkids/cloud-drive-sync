...
In demo mode, the daemon creates sample files and simulates sync activity. This is useful for developing the UI or testing the full flow without Google credentials.
...
Each `[[sync.pairs]]` entry defines a local-to-remote folder mapping. You can have multiple entries.
