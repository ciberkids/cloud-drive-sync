...
> The HTTP and MCP front-ends each take an optional shared token (`--http-token` / `--mcp-token`); both are unauthenticated without one, and both bind all interfaces. The HTTP front-end additionally supports **one web UI account** — a username and password for the browser, layered beside the token rather than replacing it, so nothing automated changes when you create one. The account is a database row; its sessions are in process memory. See [Authentication](Daemon#authentication).
